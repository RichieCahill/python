...
class Universe:
    ...
    def find_empty_rows(self) -> list[int]:
        """find_empty_lines"""
        return {index for index, line in enumerate(self.input_data) if len(set(line)) == 1 and set(line) == {"."}}
    ...
    def get_distance(self, galaxy1: Galaxy, galaxy2: Galaxy) -> int:
        ...
        if test := len(column_intersection):
            add_test += (test * self.expansion_factor) - test
        return abs(galaxy1.row - galaxy2.row) + abs(galaxy1.column - galaxy2.column) + add_test
